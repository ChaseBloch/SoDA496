...
df_Net.rename(
    columns = {'Close/Last':'close'}, inplace = True 
    ) 
...
df_Net['Date'] = [
    datetime.strptime(x, '%m/%d/%Y').date() for x in df_Net['Date']
    ] 
...
for j in range(len(df_Net)):
    for i,tweet in enumerate(
        ...
        attributes_container.append(
            [tweet.user.username, 
             tweet.date, tweet.likeCount, 
             tweet.sourceLabel, tweet.rawContent]
            )
...
tweets_df['Date'] = [
    datetime.strptime(x[0:10], '%Y-%m-%d').date() 
    for x in tweets_df['Date Created']
    ]
...
# Plot a time series graph of sentiment and Netflix's stock performance.
...
# Create a scatterplot of the relationship between Netflix sentiment and stock.
